...
for i in range(0,N):
	...
	text_3_pipe_removed = re.sub(r'\|\|\|', ' ', text_post)
	text_urls_removed = re.sub(r'(http\S+)', ' ', text_3_pipe_removed)
	text_numbers_removed = re.sub(r'[0-9]+', ' ', text_urls_removed)
	mbti_raw_data.iloc[i]['posts'] = text_numbers_removed

mbti_raw_data.to_csv('mbti_data_scrub.csv')
